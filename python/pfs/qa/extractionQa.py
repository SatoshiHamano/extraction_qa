...
from pfs.datamodel import PfsDesign, FiberStatus, TargetType, PfsConfig
...
class ExtractionQaConnections(
    ...
    fiberProfiles = InputConnection(
        name="fiberProfiles",
        doc="1D profile functions",
        storageClass="FiberProfiles",
        dimensions=("instrument", "exposure", "detector"),
        multiple=True,
    )
...
class ExtractionQaTask(CmdLineTask, PipelineTask):
    ...
    def runDataRef(self, dataRef: ButlerDataRef) -> Struct:
        """Calls ``self.run()``
        ...
        return self.run(calexp, fiberProfiles, detectorMap, pfsArm, pfsConfig)
    ...
    def getStatsPerFiber(data, detectorMap, fiberId, xwin=3):
        ymin = 0
    ...
    def run(
        ...
        Detail plots : `extQA_image_{:06}-{}{}.pdf`
            The calexp, residual, and chi images and the comparison of the calexp profile and fiberProfiles are
            plotted for some fibers with bad extraction quality.
        ...
        targetMask = {}
        if configBool:
            msk = (pfsConfig.spectrograph == dataId["spectrograph"]) * (
        ...
        for fiberId in fiberIds:
            chi2, _, x_ave, chi_f, mask_f = self.getStatsPerFiber(
                data, detectorMap, fiberId, xwin=self.config.fiberWidth
            )
            ...
            chiPeak.append(
                np.average(chi_f[:, self.config.fiberWidth][mask_f[:, self.config.fiberWidth] == 0])
            )
        ...
        if any(np.array(chiStd) > self.config.thresChi):
            pp = PdfPages("/%(visit:06)d_%(arm)s%(spectrograph)d_chifiber.pdf" % dataId)
            for fiberId in fiberIds:
        ...
        statsPickle = open("extQA_image_%(visit:06)d-%(arm)s%(spectrograph)d.pickle" % dataId, "wb")
        ...
        pp = PdfPages("extQA_image_%(visit:06)d-%(arm)s%(spectrograph)d.pdf" % dataId)
